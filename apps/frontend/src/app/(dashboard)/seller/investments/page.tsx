...
import {
  ...
  BarChart3, Users, Loader2, Search,
} from 'lucide-react'
...
export default function SellerInvestmentsPage() {
  ...
  const totalRaised   = investments.filter(i => i.status === 'APPROVED').reduce((s, i) => s + i.amount, 0)
  const totalPending  = investments.filter(i => i.status === 'PENDING').reduce((s, i) => s + i.amount, 0)
  ...
  // Build a simple 30-day trend from approved investments
  const trendData = (() => {
    const approved = investments.filter(i => i.status === 'APPROVED')
    const now = Date.now()
    const days = Array.from({ length: 30 }, (_, i) => {
      const dayTs = now - (29 - i) * 86400000
      const cumulative = approved
        .filter(inv => {
          const ts = inv.createdAt?.toDate ? inv.createdAt.toDate().getTime() : new Date(inv.createdAt).getTime()
          return ts <= dayTs
        })
        .reduce((s, inv) => s + inv.amount, 0)
      return { date: new Date(dayTs).toISOString().split('T')[0], value: cumulative }
    })
    return days
  })()
  ...
  return (
    <DashboardLayout role="SELLER" title="Investment Allocation" subtitle="Track investments received from investors">

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Raised',      value: fmt(totalRaised),   icon: DollarSign,   color: 'text-brand-400',   bg: 'bg-brand-500/10' },
          { label: 'Pending Capital',   value: fmt(totalPending),  icon: Clock,        color: 'text-amber-400',   bg: 'bg-amber-500/10' },
          { label: 'Active Investors',  value: uniqueInvestors,    icon: Users,        color: 'text-violet-400',  bg: 'bg-violet-500/10' },
          { label: 'Approved Deals',    value: approvedCount,      icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
        ].map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div>
              <p className={cn('text-xl font-bold font-display font-mono', s.color)}>{s.value}</p>
      ...
      {totalRaised > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-display font-bold">Capital Raised</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Cumulative approved investment over 30 days</p>
            </div>
          ...
          <AreaChart data={trendData} height={200} color="#10b981" format="currency" />
        </motion.div>
      )}
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-2xl flex flex-col items-center justify-center py-20 gap-4">
          ...
          <div className="text-center">
            <p className="font-semibold">{search || filter !== 'ALL' ? 'No results' : 'No investments yet'}</p>
            <p className="text-sm text-muted-foreground mt-1">Investments will appear here once investors submit requests</p>
          </div>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider px-4 py-4 hidden lg:table-cell">Terms</th>
                    ...
                    <motion.tr key={inv.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                      ...
                      <td className="px-4 py-4 hidden lg:table-cell">
                        <p className="text-xs text-muted-foreground">
                          ...
                          {inv.lockPeriod  ? ` · ${inv.lockPeriod}mo` : ''}
                        </p>
                      </td>
                      <td className="px-4 py-4">

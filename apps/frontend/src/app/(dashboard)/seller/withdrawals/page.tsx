...
import { ArrowDownToLine, CheckCircle, XCircle, DollarSign, Clock, AlertCircle, Filter, X } from 'lucide-react'
...
import { useSellerStore, type SellerWithdrawal } from '@/stores/seller.store'
...
function RejectModal({ withdrawal, onConfirm, onClose }: { withdrawal: SellerWithdrawal; onConfirm: (notes: string) => void; onClose: () => void }) {
...
export default function SellerWithdrawalsPage() {
  const { withdrawals, approveWithdrawal, rejectWithdrawal, markWithdrawalCompleted } = useSellerStore()
  const [filter, setFilter] = useState<FilterStatus>('ALL')
  const [rejectTarget, setRejectTarget] = useState<SellerWithdrawal | null>(null)
  ...
  const COLS: Column[] = [
    ...
    { key: 'amount', label: 'Amount', sortable: true, render: (v) => <span className="font-mono font-semibold">{formatCurrency(v)}</span> },
    {
      ...
      render: (v) => <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full', METHOD_BADGE[v] || 'bg-obsidian-800 text-muted-foreground')}>{v.replace('_', ' ')}</span>,
    },
    ...
    { key: 'requestedAt', label: 'Requested', render: (v) => <span className="text-xs text-muted-foreground">{formatDate(v)}</span> },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-1.5 justify-end">
          {row.status === 'PENDING' && (
            <>
              <Button size="sm" variant="profit" className="h-7 text-xs gap-1" onClick={() => { approveWithdrawal(row.id); toast.success(`Approved for ${row.investorName}`) }}>
  ...
  return (
    <DashboardLayout role="SELLER" title="Withdrawal Requests" subtitle="Review and approve investor withdrawal requests">
      {pendingCount > 0 && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-center gap-3 p-4 rounded-xl border border-amber-500/30 bg-amber-500/5">
          ...
          <p className="text-sm text-amber-300">
            <span className="font-semibold">{pendingCount} withdrawal requests</span> awaiting your approval — totalling{' '}
      ...
      <AnimatePresence>
        {rejectTarget && (
          <RejectModal withdrawal={rejectTarget} onConfirm={(notes) => { rejectWithdrawal(rejectTarget.id, notes); toast.success('Withdrawal rejected') }} onClose={() => setRejectTarget(null)} />
        )}
      </AnimatePresence>
    </DashboardLayout>
  )
